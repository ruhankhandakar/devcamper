...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  const publishedBootcamp = await Bootamp.findOne({ user: req.user.id });

  // If user is not an admin, then they only can add one bootcamp
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  let bootcamp = await Bootamp.findById(req.params.id);
  ...
  bootcamp = await Bootamp.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true
  });
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async err => {
    ...
    await Bootamp.findByIdAndUpdate(req.params.id, {
      photo: file.name
    });
